(function () {
  const LEGACY_STORAGE_KEY = "seating_viewer_behavior_log_v1";
  ...
  function initSeatBehaviorBridge() {
    ...
    if (window.BehaviorStore?.migrateLegacyEvents) {
    ...
    window.seatBehaviorBridge = buildBridgeApi();
  ...
  function handleBehaviorLog(payload) {
    const selectedContexts = getSelectedSeatContexts();
    const targets = selectedContexts.length > 1
      ? selectedContexts
      : [selectedContexts[0] || BRIDGE_STATE.lastSeatContext || null];

    targets.forEach((seatContext) => {
      const mergedPayload = {
        ...buildSeatContextDefaults(seatContext),
        ...payload,
        studentId: seatContext?.studentId || seatContext?.studentNumber || payload?.studentId || "",
        studentName: seatContext?.studentName || seatContext?.student || payload?.studentName || "",
        studentNumber: seatContext?.studentNumber || payload?.studentNumber || "",
        periodId: seatContext?.periodId || payload?.periodId || "",
        seatId: seatContext?.seatId ?? payload?.seatId ?? "",
        tableId: seatContext?.tableId ?? payload?.tableId ?? "",
        zone: seatContext?.zone || payload?.zone || "",
        position: seatContext?.position || payload?.position || "",
        matchedRosterName: seatContext?.matchedRosterName || payload?.matchedRosterName || "",
        rosterIndex: seatContext?.rosterIndex || payload?.rosterIndex || "",
        grade: seatContext?.grade || payload?.grade || "",
        loggedAt: payload?.loggedAt || new Date().toISOString(),
      };

      if (window.BehaviorStore?.addEvent) {
        const result = window.BehaviorStore.addEvent(mergedPayload);
        if (result?.ok && result.event) {
          BRIDGE_STATE.behaviorLog = loadBehaviorLog();
          document.dispatchEvent(new CustomEvent("behavior:logged", { detail: result.event }));
          return;
        }
      }
      ...
      trimBehaviorLog();
      saveBehaviorLog();
      document.dispatchEvent(new CustomEvent("behavior:logged", { detail: mergedPayload }));
    });

    if (window.BehaviorStore?.rebuildDashboardCache) {
  ...
  function getSelectedSeatContexts() {
    ...
    if (!viewerState || !viewerState.data || !selectedSeatIds) return [];
    ...
    if (!periodData || !Array.isArray(periodData.assignments)) return [];

    const selectedIds = Array.isArray(selectedSeatIds) ? selectedSeatIds.map(Number) : [];
    if (!selectedIds.length) return [];

    const assignmentMap = new Map(periodData.assignments.map((assignment) => [Number(assignment.seatId), assignment]));
    const layoutMap = new Map(layoutSeats.map((seat) => [Number(seat.seatId), seat]));
  ...
  function getBehaviorLog() {
    BRIDGE_STATE.behaviorLog = loadBehaviorLog();
    return [...BRIDGE_STATE.behaviorLog].sort((a, b) => {
      const aTime = Date.parse(b.loggedAt || b.timestamp || 0) || 0;
      const bTime = Date.parse(a.loggedAt || a.timestamp || 0) || 0;
      return aTime - bTime;
    });
  }
  ...
  function clearBehaviorLog() {
    ...
    document.dispatchEvent(new CustomEvent("behavior:hydrated", { detail: { count: 0 } }));
  }

  function saveBehaviorLog() {
    try {
      localStorage.setItem(LEGACY_STORAGE_KEY, JSON.stringify(BRIDGE_STATE.behaviorLog));
    } catch (error) {
      console.warn("[seat-behavior-bridge] Failed to save legacy behavior log:", error);
    }
  }

  function loadBehaviorLog() {
    if (window.BehaviorStore?.getEvents) {
      const events = window.BehaviorStore.getEvents();
  ...
  function clearSavedBehaviorLog() {
    if (window.BehaviorStore?.clearEvents) {
      window.BehaviorStore.clearEvents();
    }
    try {
